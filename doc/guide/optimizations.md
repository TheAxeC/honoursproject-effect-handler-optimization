# Optimizations in the Eff compiler

The optimizations are all written in a single file: `optimize.ml`.  
A file is compiled into an AST. All top-level commands are put into a list.  
Compiling commands can be done with:
```ocaml
let optimize_commands cmds =
  List.map (fun (cmd, loc) -> (optimize_command cmd, loc)) cmds
```
A single command can be compiled using:
```ocaml
...
The most important methods are the reduce methods:
Expressions:
```ocaml
and reduce_expr e =
  match e.term with
